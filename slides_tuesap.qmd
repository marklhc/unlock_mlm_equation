...
::: {.callout}
- Standardized effect size [@lai2014; @lai2016; @lai2021]
- Cross-classified models [@lai2014; @lai2019]
- Bootstrapping [@lai2021; @luo2021]
- Survey sampling [@lai2018; @luo2021]
- Measurement invariance [@hsiao2018]
- Reliability [@lai2021a]
- Heteroscedasticity [@zhang2024]
:::
...
- With unequal cluster means, $\bv Z_1^\top \bv Z_0$ $\neq$ $\bv 0$ and $\bv Z_1^\top \bv M_{Z_0}$ $\neq$ $\bv 0$
...
- For general level-1 predictor
    * Random slope variance ($\tau_1^2$), if omitted, will be redistributed to both the random intercept variance ($\tau_0^2$) and $\sigma^2$
:::
...
- For purely level-1 predictor, $\bv Z_1^\top \bv Z_0$ = $\bv 0$
    * Random slope variance ($\tau_1^2$), if omitted, will be redistributed to only $\sigma^2$
:::
...
Using simulation, we found that, when clustering is ignored, SE of the following is underestimated 
...
$$
\sigma^2 [\Tr(\bv D \bv K_Z) + 1] = 1 - R^2_\text{GLMM}
$$
...
## Additional Notes on SMD {.smaller}
...
## Additional Notes on SMD (cont'd)
...
- $\gamma^s_\text{ses}$ = `r round(fixef(m1)[2] * sd(Hsb82$ses) / sqrt(fe + sum(Kz * tau_mat) + sigma(m1)^2), 2)`
- $\gamma^s_\text{meanses}$ = `r round(fixef(m1)[3] * sd(Hsb82$ses) / sqrt(fe + sum(Kz * tau_mat) + sigma(m1)^2), 2)`
:::

## $f$ and $f^2$ Effect Size

$$
f^2_\text{GLMM} = \frac{R_\text{GLMM}^2}{1 - R_\text{GLMM}^2}
$$

- For two-group designs, $2f$ = $d$
...
$$
\begin{aligned}
\mathrm{Deff}(\hat \gamma_k) & = \frac{{\bv \Sigma^{-1}_{X}}_{kk} + \tilde n \left[\bv \Sigma^{-1}_X \bv G \bv \Sigma^{-1}_X\right]_{kk}}{{\bv \Sigma^{-1}_X}_{kk} \Tr(\bv D \bv K_Z) + 1} \\
& = (1 - \textrm{ICC}_D) + (1 - \textrm{ICC}_D) \tilde n \frac{\left[\bv \Sigma^{-1}_X \bv G \bv \Sigma^{-1}_X\right]_{kk}}{{\bv \Sigma^{-1}_X}_{kk}}
\end{aligned}
$$
...
# Power Analysis {background-color="#595959"}
...
- How can we obtain $\Delta R^2$ and CI?
- How does omitting $X$ affect estimates of $\gamma$ and $\tau$?
- How does omitting $Z$ affect estimates of $\gamma$ and $\tau$?
- How do we generalized beyond two-level models?
    * E.g., Crossed random effects [e.g., @lai2019]
:::
