...
## My Journey in Multilevel Modeling (MLM) {visibility="hidden"}
...
## The Journey Continues . . . {visibility="hidden"}
...
- Bootstrapping
- Survey sampling
- Measurement (invariance, reliability, factor analysis)
- Heteroscedasticity
:::
...
utex <- outer(1:2, Y = 0:1, FUN = \(x, y) paste0("\\color{blue}{{u_{", y, x, "}}}")) |>
    paste0(collapse = " \\\\")
etex <- outer(1:4, Y = 1:2, FUN = \(x, y) paste0("e_{", y, x, "}")) |>
    paste0(collapse = " \\\\")
...
- $\bv e$ is a vector of errors;^[or $V(\bv e) = \sigma^2 \bv R$ for autocorrelations/heteroscedasticity] $V(\bv e) = \sigma^2 \bv I$
- $\bv u$: vector of length $q J$ of random effects; $V(\bv u_j) = \bv T = \sigma^2 \bv D \quad \forall j$\
    $$
    \bv D = \begin{bmatrix}
      \tau_0^2 \color{gold}{{/ \sigma^2}} & & & \\
      \tau_{01} \color{gold}{{/ \sigma^2}} & \tau_1^2 \color{gold}{{/ \sigma^2}} & & \\
      \vdots & \cdots & \ddots & \\
      \tau_{0 q -1} \color{gold}{{/ \sigma^2}} & \tau_{1 q -1} \color{gold}{{/ \sigma^2}} & \cdots & \tau^2_{q - 1} \color{gold}{{/ \sigma^2}} \\
    \end{bmatrix}
    $$
